...
class BenchmarkSuite:
    ...
    async def run_benchmark(
        self,
        train_file: str = "evals/train.json",
        test_file: str = "evals/test.json",
...
""")
    ...
    if command == "run":
        ...
        run = await suite.run_benchmark(
            train_file="evals/train.json",
            test_file="evals/test.json",
        ...
        if not runs:
            ...
            print(f"\n{'Run ID':<30} {'Timestamp':<17} {'Model':<12} {'Version':<10} {'Queries':<8} {'Tokens':<10} {'Tools'}")
            print("-" * 100)
            for r in runs:
                model_short = r['model'].replace('claude-', '').replace('-20250514', '')[:10]
                print(f"{r['run_id']:<30} {r['timestamp'][:16]:<17} {model_short:<12} {r['agent_version']:<10} {r['success_rate']:<8} {r['avg_tokens']:<10} {r['avg_tools']}")
